...
def _add_streak_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    run_len_incl = (
        df.groupby(
            [df["pitcher_id"], df["game_id"], block_id],
            sort=False,
        ).cumcount()
        + 1
    )
...
def _compute_pitcher_biomech(
    ...
    stats["pitcher_avg_extension"] = stats["pitcher_avg_extension"].astype("float32")
    stats["pitcher_avg_arm_angle"] = stats["pitcher_avg_arm_angle"].astype("float32")
    return stats
