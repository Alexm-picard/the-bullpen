...
def main() -> None:
    ...
    proba = np.asarray(
        booster.predict(feature_matrix(emb_te, test_df, ALL_FEAT)), dtype=np.float32,
    )
    m = compute_pitch_type_metrics(
        "Hybrid + Context", y["test"], proba, t_time + (time.perf_counter() - t0),
    )
    ...
    if save_dir is not None:
        ...
        (save_dir / "v2_id_maps.json").write_text(
            json.dumps({"pitcher_map": pm, "batter_map": bm})
        )
        update_metadata(
            save_dir, v2_transformer=v2_arch_meta(model, cfg), results=[metric_dict(m)],
        )
