"""Python side of the Python↔Java parity test for pitch_outcome_post (Phase 2b.3).

Sister to test_python_java_parity_pre.py. Loads the committed fixture
(`tests/fixtures/parity_pitch_post_001*.json`) and re-runs every input
row through Python's ONNX Runtime + isotonic calibrator + the same
preprocess logic the Java side implements. Each output must match the
expected value within `tolerance` (1e-6) at every stage.

The Java mirror is `backend/.../inference/PitchPostParityTest.java`;
both sides read the same JSON files so the fixture is the contract.
"""
...
from bullpen_training.pitch.parity_fixture_post import (
    _load_pitch_type_mapping,
    _preprocess,
)

REPO_ROOT = Path(__file__).resolve().parents[2]
CONTRACT_PATH = REPO_ROOT.parent / "contracts" / "feature_pipeline_post.json"
FIXTURE_DIR = REPO_ROOT / "tests" / "fixtures"
INPUT_PATH = FIXTURE_DIR / "parity_pitch_post_001.json"
EXPECTED_PATH = FIXTURE_DIR / "parity_pitch_post_001_expected.json"
ARTIFACT_DIR = REPO_ROOT / "artifacts" / "pitch_outcome_post" / "v1"
ONNX_PATH = ARTIFACT_DIR / "model.onnx"
CALIBRATOR_PATH = ARTIFACT_DIR / "calibrator.json"
PARK_PATH = ARTIFACT_DIR / "park_id_mapping.json"
PITCH_TYPE_PATH = ARTIFACT_DIR / "pitch_type_mapping.json"
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        "Post parity fixture or production artifact missing — run "
        "`bullpen_training.pitch.production --model post --version v1` "
        "then `bullpen_training.pitch.export_post_onnx` then "
        "`bullpen_training.pitch.parity_fixture_post` to populate."
    ),
)
...
def test_input_and_expected_share_schema_hash() -> None:
    ...
    assert inp["schema_hash"] == exp["schema_hash"], (
        ...
        "regenerate both with parity_fixture_post"
    )
