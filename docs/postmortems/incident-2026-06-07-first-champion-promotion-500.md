...
- **Status**: **resolved** (2026-06-07). Clean recovery completed on deploy `v2026.06.07-1231` (`2b320ec`): v1 `battedball_outcome` is CHAMPION serving `/all-parks` 200 (`modelVersion v1`, 30 parks) through the registry, routing `champion_version_id=1`, and `prediction_log` is confirmed flowing (load -> inference -> per-park calibration -> response -> async log). Durable fixes `[149]`/`[150]`/`[151]`/`[152]` all deployed. Recovered without ever registering a v2 (INC-6 respected). NOTE: the INC-2 load gate caught a fourth, code-side layer at the recovery re-promote - the reader fed the input name `"input"` while the MLP names it `"features"` (`[152]`); fixed in the reader, NOT the snapshot.
...
| later  | Mac: INC-1 champion rollback (`[150]`/`2878420`) + INC-2 promote load gate (`[151]`/`27298ac`) landed - the two machinery rails the incident exposed.                                                                                                                                                                                          |
| ~11:54 | Recovery deploy `v2026.06.07-1154` (rails). Re-promote attempts held v1 safely at SHADOW by the INC-2 load gate: first the missing `model.onnx.data` sidecar, then `Unknown input name "input", expected ["features"]` - the latent reader/exporter input-name mismatch the gate surfaced (caught by the gate, not a live 500).                |
| later  | Mac: reader input-name fix (`[152]`/`2ca7e2f`) - `BattedBallOnnxModel` resolves the input name from the session (the MLP names it `"features"`; the reader had hardcoded `"input"`). Latent + original, not a `[148]` regression.                                                                                                              |
| ~12:31 | Recovery deploy `v2026.06.07-1231` (`2b320ec`; INC-1 + INC-2 + reader fix). Snapshot verified complete (calibrator map-format, sidecar 151 KB, model input `["features"]`). Demote v1 (INC-1) -> re-promote v1: INC-2 gate PASSED (model loads + predicts). **C5: `/all-parks` 200, `modelVersion v1`, 30 parks.**                             |
| ~12:3x | Independent box verification (read-only): `battedball_outcome` v1 CHAMPION, routing `champion_version_id=1` (real model, not toy fallback), JAR `v2026.06.07-1231` @ `2b320ec`, `prediction_log` flowing (`battedball_outcome`/v1/champion). **Incident resolved** - first real model served through the registry end-to-end.                  |
...
| Item                                                                                                                                                                                 | Owner                    | Status                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Re-key the on-box v1 calibrator list->map (interim unblock)                                                                                                                          | box-operator             | **DONE** (2026-06-07; `calibrator.json` `schema_version` 2 confirmed on box) via `convert_calibrator_list_to_map.py`                                                                           |
...
| INC-2 register/promote **load gate** (load + warmup predict; reject on failure)                                                                                                      | registry track           | **DONE** (decision `[151]`, commit `27298ac`; deployed `v2026.06.07-1231`)                                                                                                                     |
| INC-1 champion **rollback** path (CHAMPION -> SHADOW or non-terminal RETIRED)                                                                                                        | registry track           | **DONE** (decision `[150]`, commit `2878420`; deployed `v2026.06.07-1231`)                                                                                                                     |
| INC-4 hash the calibrator under rule 7 (content guard)                                                                                                                               | registry track           | proposed (deferred; not in the recovery deploy)                                                                                                                                                |
| INC-7 reader resolves the ONNX input name from the session (MLP names it `"features"`, reader fed `"input"`; latent + original, caught by the INC-2 gate at the recovery re-promote) | developer (Mac)          | **DONE** (decision `[152]`, commit `2ca7e2f`; deployed `v2026.06.07-1231`)                                                                                                                     |
| Clean recovery: verify snapshot complete -> (INC-1) demote v1 -> (INC-2-gated, [152]-dependent) re-promote v1 -> C5                                                                  | developer + box-operator | **DONE** (2026-06-07, deploy `v2026.06.07-1231`/`2b320ec`): v1 CHAMPION, `/all-parks` 200 `modelVersion v1`, routing `champion_version_id=1`, `prediction_log` flowing; box-verified read-only |
