...
- Uptime Robot monitoring `/health`
...
- **S3-compatible object storage online** (per ADR-0007, with prod endpoint revised to R2 by decision [128]) — Cloudflare R2 bucket(s) provisioned (`bullpen-prod`, with `samples/dev/`, `snapshots/v{N}/`, `raw/` prefixes); `rclone` configured against the R2 S3-compatible endpoint (`https://<account-id>.r2.cloudflarestorage.com`); `S3_ENDPOINT_URL` env var wired through both the Spring app and the Python training code. On the MacBook, **MinIO** runs from `/Volumes/MyDrive/bullpen-data` for offline dev (read-only `localhost:9000`), launched via `make minio-up` / `make minio-down`. `make sync-mirror` runs `rclone sync` from R2 to the portable drive — the pre-travel ritual. No code anywhere reads `file://` storage paths; the only environment-specific knob is the endpoint URL.
...
12. **All object storage via S3-compatible client with `S3_ENDPOINT_URL` as the only environment-specific knob.** Cloudflare R2 in prod (vendor-consolidated with Tunnel + DNS, per decision [128]); MinIO on the portable drive for offline dev. No `file://` paths in storage code, no second abstraction. (ADR-0007)
