...
class RateLimitFilterTest {
  ...
  void predict429sWithApiErrorEnvelopeOnceBudgetExhausted() throws Exception {
    MockMvc mvc = mvcWith(new RateLimitFilter(true, 3, 120, new ObjectMapper()));
    for (int i = 1; i <= 3; i++) {
  ...
  void predictAndSearchHaveIndependentBudgets() throws Exception {
    MockMvc mvc = mvcWith(new RateLimitFilter(true, 1, 2, new ObjectMapper()));
    mvc.perform(post("/v1/predict/ping")).andExpect(status().isOk());
  ...
  void unthrottledPathsNeverLimited() throws Exception {
    MockMvc mvc = mvcWith(new RateLimitFilter(true, 1, 1, new ObjectMapper()));
    for (int i = 0; i < 5; i++) {
  ...
  void disabledFilterPassesEverythingThrough() throws Exception {
    MockMvc mvc = mvcWith(new RateLimitFilter(false, 1, 1, new ObjectMapper()));
    for (int i = 0; i < 5; i++) {
      mvc.perform(post("/v1/predict/ping")).andExpect(status().isOk());
    }
  }
