 ...
 * CHAMPION-stage version OF ITS OWN MODEL (the scan's JOIN binds model_name - rule 9). The V020
 ...
 * or a row stranded by a stage flip that predates this guard.
 *
 ...
 * out-of-band {@code sqlite3} delete of the offending row on the box (with a {@code .backup} first
 * - the documented emergency exception to ADR-0006's read-only rule), or restoring {@code
 * registry.sqlite} from the nightly snapshot's {@code _sqlite/} capture. Never weaken the check
 * itself.
 *
...
public class RoutingChampionIntegrityCheck implements SmartInitializingSingleton {
  ...
  public void afterSingletonsInstantiated() {
    // One read: the checked-count and the violation scan come from the same rows, so the log
    // never reports a count from a different table state than the one that was checked.
    List<RoutingRepository.ChampionStageRow> rows = repo.findChampionStageRows();
    List<RoutingRepository.ChampionStageRow> violations =
        rows.stream().filter(RoutingRepository.ChampionStageRow::violates).toList();
